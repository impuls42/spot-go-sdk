...
// TestCreateVMPool_WithUserData tests the CreateVMPool function with VMUserData using a mocked SpotClient.
func TestCreateVMPool_WithUserData(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	mockSpotAPI := mocks.NewMockSpotAPI(ctrl)

	expectedUserData := "IyEvYmluL2Jhc2gKZWNobyAnSGVsbG8gZnJvbSBjbG91ZC1pbml0JwphcHQtZ2V0IHVwZGF0ZSAteQ=="

	// Define the behavior of the mock CreateVMPool method
	mockSpotAPI.EXPECT().CreateVMPool(gomock.Any(), "test-org", gomock.Any()).DoAndReturn(func(ctx context.Context, org string, pool v1.VMPool) error {
		if pool.VMUserData != expectedUserData {
			return fmt.Errorf("expected VMUserData %s, got %s", expectedUserData, pool.VMUserData)
		}
		return nil
	})
	...
	pool := v1.VMPool{
		Name:         "test-vm-pool",
		Org:          "test-org",
		VMCloudSpace: "test-vm-cloudspace",
		ServerClass:  "gp.vs2.medium-sjc",
		Desired:      1,
		BidPrice:     "0.97",
		PoolType:     "spot",
		VMImage:      "ubuntu24.04",
		VMUserData:   expectedUserData,
	}

	// Call the CreateVMPool function
	err := mockSpotAPI.CreateVMPool(context.Background(), "test-org", pool)

	if err != nil {
		t.Fatalf("Failed to create VM pool: %v", err)
	}
}
